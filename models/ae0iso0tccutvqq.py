...
class GAN(BaseModel):
    ...
    def validation_step(self, batch, batch_idx):
        if batch_idx == 0:
            ...
            self.log('val_vq', self.qloss, on_step=False, on_epoch=True,
                     logger=True, sync_dist=True)
            ...
            if val_l1 < self.best_val_loss:
                ...
                if client is not None:
                    client.log_metric(run_id, 'best_val_l1',
                                      self.best_val_loss, step=self.epoch)
                    client.log_metric(run_id, 'best_epoch',
                                      self.best_epoch, step=self.epoch)
