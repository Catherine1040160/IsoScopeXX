...
class GAN(BaseModel):
    ...
    def __init__(self, hparams, train_loader, eval_loader, checkpoints):
        ...
        self.upsample = torch.nn.Upsample(size=(hparams.cropsize, hparams.cropsize, hparams.cropz // hparams.dsp * hparams.uprate), mode='trilinear', align_corners=True)
    ...
    def generation(self, batch):
        ...
        self.rawX = batch['img'][0]

        if self.hparams.downz > 0:
            batch['img'][0] = torch.nn.Upsample(scale_factor=(1, 1, 1 / self.hparams.downz), mode='trilinear', align_corners=True)(batch['img'][0])
            batch['img'][0] = torch.nn.Upsample(scale_factor=(1, 1, self.hparams.downz), mode='trilinear', align_corners=True)(batch['img'][0])
        ...
        # X-Y permute
        self.xy_permuted = np.random.randint(2) == 1
        if self.xy_permuted:
        ...
        self.rawX = self.rawX[:, :, :, :, self.z_init:self.z_init + self.hparams.cropz]
        if self.xy_permuted:
            self.rawX = self.rawX.permute(0, 1, 3, 2, 4)
        self.rawX = self.upsample(self.rawX)
    ...
    def backward_g(self):
        ...
        if self.hparams.skipl1 == 1:
            loss_l1 = self.add_loss_l1(a=self.get_projection(self.XupX, depth=1, how=self.hparams.l1how),
                                    b=self.Xup[:, :, :, :, :])
        else:
            loss_l1 = self.add_loss_l1(a=self.get_projection(self.XupX, depth=self.hparams.skipl1, how=self.hparams.l1how),
                                    b=self.Xup[:, :, :, :, self.hparams.skipl1//2::self.hparams.skipl1])
        ...
        # PSNR between rawX and XupX

        mse = torch.mean((self.rawX - self.XupX) ** 2)
        psnr = 10 * torch.log10(torch.tensor(4.0, device=mse.device) / (mse + 1e-8))
        loss_dict['psnr'] = psnr
